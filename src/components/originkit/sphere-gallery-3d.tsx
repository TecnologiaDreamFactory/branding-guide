...
export interface GalleryItem {
    image?: string
    link?: string
    /** cor chapada da esfera 3D do nó; a imagem vira só o rótulo por cima */
    color?: string
}
...
export interface SphereGallery3DProps {
    ...
    rounded?: number
    core?: CoreGroup
    /** clique num nó: recebe o índice. Substitui o window.open do original. */
...
void main() {
    ...
    vec3 col = mix(uPlaceholder, tex.rgb, uHasTex);
    col *= uDim;

    float a = mask * uOpacity * mix(1.0, tex.a, uHasTex);
    if (a <= 0.002) discard;
...
/* ------------------------------------------------------------- cubo do nó
 * 6 faces, 2 triângulos cada, meia-aresta 1. As faces ficam CONTÍGUAS no
 * buffer pra cada uma sair num drawArrays próprio — é assim que cada face
 * recebe seu tom chapado sem tocar no shader.
 */
const CUBE_FACES: ReadonlyArray<readonly [number, number, number]> = [
    [1, 0, 0],
    [-1, 0, 0],
    [0, 1, 0],
    [0, -1, 0],
...
/** direção da luz, FIXA no mundo: é ela que faz as faces trocarem de tom
 *  conforme o cubo gira, em vez de parecer pintura presa no objeto */
const CUBE_LIGHT = [-0.32, 0.55, 0.77]
/** face de costas para a luz; 1 = face de frente. Degraus chapados, sem rampa */
const CUBE_FLOOR = 0.6

function buildCube(): Float32Array {
    const pos: number[] = []
    for (const n of CUBE_FACES) {
        // tangente qualquer que não seja paralela à normal
        const t = Math.abs(n[1]) === 1 ? [1, 0, 0] : [0, 1, 0]
        // u = t x n, v = n x u -> u x v = n, então a ordem abaixo sai CCW
        // vista DE FORA e o cull de face traseira funciona
        const u = [
            t[1] * n[2] - t[2] * n[1],
            t[2] * n[0] - t[0] * n[2],
            t[0] * n[1] - t[1] * n[0],
...
export default function SphereGallery3D(props: SphereGallery3DProps) {
     ...
     * the props array.
     */
    const onSelectRef = React.useRef<typeof onSelect>(undefined)
    onSelectRef.current = onSelect

    const colorsRef = React.useRef<string[]>([])
    colorsRef.current = Array.from(
        { length: branches },
        (_, i) => items[i % items.length]?.color || ""
    )
    ...
    React.useEffect(() => {
        ...
        const upload = (img: HTMLImageElement): WebGLTexture | null => {
            ...
            gl.texImage2D(
                gl.TEXTURE_2D,
                0,
                // gl.RGBA, NÃO SRGB8_ALPHA8: com o decode sRGB->linear em
                // hardware o hex da marca chegava na tela escurecido. Aqui os
                // cards são cor chapada de marca, então tem de bater exato.
                gl.RGBA,
                gl.RGBA,
                gl.UNSIGNED_BYTE,
                img
        ...
        const onPointerMove = (e: PointerEvent) => {
            ...
            if (!pointer.dragging) {
                if (inBox) {
                    ...
                    canvas.style.cursor =
                        idx >= 0 && (onSelectRef.current || linksRef.current[idx])
                            ? "pointer"
                            : "grab"
                }
                return
        ...
        const frame = () => {
            ...
            /* --------------------------------------------------- cubos
             * Um nó é um CUBO 3D. Opaco e com escrita de profundidade — é
             * isso que faz um cubo tapar o outro por z-buffer em vez de
             * depender do sort.
             *
             * Cada face sai num drawArrays próprio com um tom CHAPADO da cor
             * da marca: sem isso as 6 faces têm a mesma cor, as arestas somem
             * e o cubo lê como um hexágono liso. O tom vem da normal da face
             * girada pra o mundo contra uma luz fixa, então é o cubo que gira
             * sob a luz, e não a pintura que gira com ele.
             */
            const faceShade = new Float32Array(6)
            for (let f = 0; f < 6; f++) {
                const nf = CUBE_FACES[f]
                applyBasis(R, nf[0], nf[1], nf[2], tmpA)
                const d =
                    tmpA[0] * CUBE_LIGHT[0] +
                    tmpA[1] * CUBE_LIGHT[1] +
                    tmpA[2] * CUBE_LIGHT[2]
                faceShade[f] = CUBE_FLOOR + (1 - CUBE_FLOOR) * Math.max(0, d)
            }

            const drawCubes = (from: number, to: number) => {
                if (to <= from) return
                gl.useProgram(solidProg)
                gl.bindBuffer(gl.ARRAY_BUFFER, cubeBuf)
                gl.enableVertexAttribArray(sLoc.aPos)
                ...
                for (let o = from; o < to; o++) {
                    const i = order[o]
                    const nd = nodes[i]
                    const c = parseColor(colorsRef.current[i], [0.1, 0.11, 0.13, 1])
                    const depth = R[6] * nd.px + R[7] * nd.py + R[8] * nd.pz
                    const tt = clamp((depth + L.radius) / (2 * L.radius), 0, 1)
                    const dim = DEPTH_FLOOR + (1 - DEPTH_FLOOR) * tt

                    compose(model, nd.px, nd.py, nd.pz, IDENT_Q, 1, 1, 1)
                    mul(mvp, viewProj, model)
                    gl.uniformMatrix4fv(sLoc.uMVP, false, mvp)
                    gl.uniform1f(sLoc.uScale, nd.hy)

                    for (let f = 0; f < 6; f++) {
                        const k = dim * faceShade[f]
                        gl.uniform4f(sLoc.uColor, c[0] * k, c[1] * k, c[2] * k, 1)
                        gl.drawArrays(gl.TRIANGLES, f * 6, 6)
                    }
                }

                /* devolve o estado que os rótulos transparentes esperam */
                gl.enable(gl.BLEND)
                gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA)
                gl.depthMask(false)
                gl.disable(gl.CULL_FACE)
            ...
            for (let o = from; o < to; o++) {
                ...
                const sy = nd.hy * 2
                const sx = nd.hx * 2
                // o rótulo é plano e o nó é um CUBO: no centro ele fica dentro
                // dele e o teste de profundidade o come. Sobe até passar do
                // canto mais próximo (-forward = em direção à câmera).
                const lift =
                    nd.hy *
                    1.04 *
                    (Math.abs(forwardLocal[0]) +
                        Math.abs(forwardLocal[1]) +
                        Math.abs(forwardLocal[2]))
                compose(
                    model,
                    nd.px - forwardLocal[0] * lift,
                    nd.py - forwardLocal[1] * lift,
                    nd.pz - forwardLocal[2] * lift,
                    nd.q,
                    sx,
                    sy,
                    1
